...
export default function OperationalResultsDashboard({ variant = "result" }: { variant?: "dashboard" | "result" }) {
  ...
  const maxChart = Math.max(1, ...(result?.monthly.map((item) => Math.max(Number(item.predicted), Number(item.billed), Number(item.revenue), Number(item.expenses))) || []));
  ...
  return <main className="operational-page">
    ...
    {loading ? <ResultsSkeleton /> : result && <>
      ...
      <section className="executive-grid" aria-label="Gráficos executivos"><article className="operational-panel executive-trend"><div className="operational-panel-title"><div><span className="panel-kicker">Evolução financeira</span><h2>Previsto, faturado, recebido e custos</h2><p>Comparação mensal com o mesmo período e filtros dos indicadores.</p></div></div><div className="operational-chart" role="img" aria-label="Gráfico mensal de receita prevista, faturada, recebida e despesas">{result.monthly.map((item) => <div className="chart-column" key={String(item.month)}><div className="chart-bars"><span className="predicted" style={{ height: `${Math.max(3, Number(item.predicted) / maxChart * 100)}%` }} title={`Previsto ${money.format(Number(item.predicted))}`} /><span className="billed" style={{ height: `${Math.max(3, Number(item.billed) / maxChart * 100)}%` }} title={`Faturado ${money.format(Number(item.billed))}`} /><span className="revenue" style={{ height: `${Math.max(3, Number(item.revenue) / maxChart * 100)}%` }} title={`Recebido ${money.format(Number(item.revenue))}`} /><span className="expenses" style={{ height: `${Math.max(3, Number(item.expenses) / maxChart * 100)}%` }} title={`Despesas ${money.format(Number(item.expenses))}`} /></div><small>{String(item.month).slice(5)}/{String(item.month).slice(2, 4)}</small><b className={Number(item.operationalMargin) >= 0 ? "good" : "bad"}>{numberFormatter.format(Number(item.operationalMargin))}%</b></div>)}</div><div className="chart-legend"><span><i className="predicted" />Previsto</span><span><i className="billed" />Faturado</span><span><i className="revenue" />Recebido</span><span><i className="expenses" />Despesas</span></div></article><article className="operational-panel billing-panel"><div className="operational-panel-title"><div><span className="panel-kicker">Funil de faturamento</span><h2>Da previsão ao caixa</h2><p>Veja onde a receita ainda precisa avançar.</p></div></div><div className="billing-funnel">{[["Receita prevista", result.billingPipeline.predicted, 100], ["Faturado", result.billingPipeline.billed, result.billingPipeline.billingRate ?? 0], ["Recebido de faturas", result.billingPipeline.received, result.billingPipeline.predicted ? result.billingPipeline.received / result.billingPipeline.predicted * 100 : 0]].map(([title, value, percent]) => <div key={String(title)}><span><strong>{String(title)}</strong><small>{money.format(Number(value))}</small></span><div><i style={{ transform: `scaleX(${Math.min(1, Number(percent) / 100)})` }} /></div></div>)}</div><div className="pipeline-gaps"><div className={result.billingPipeline.toBill > 0 ? "attention" : "ok"}><span>A faturar</span><strong>{money.format(result.billingPipeline.toBill)}</strong><small>{result.billingPipeline.billingRate == null ? "Sem receita prevista" : `${numberFormatter.format(result.billingPipeline.billingRate)}% faturado`}</small></div><div className={result.billingPipeline.toCollect > 0 ? "attention" : "ok"}><span>A receber do faturado</span><strong>{money.format(result.billingPipeline.toCollect)}</strong><small>{result.billingPipeline.collectionRate == null ? "Sem faturas emitidas" : `${numberFormatter.format(result.billingPipeline.collectionRate)}% recebido`}</small></div></div></article></section>
      ...
      <section className="operational-two-columns"><article className="operational-panel"><div className="operational-panel-title"><div><span className="panel-kicker">Composição</span><h2>Distribuição dos custos</h2><p>Onde a empresa gastou no período.</p></div></div><div className="cost-list">{result.costDistribution.map((item) => <div key={item.category}><span><strong>{label(item.category)}</strong><small>{numberFormatter.format(item.percent)}%</small></span><div><i style={{ transform: `scaleX(${item.percent / 100})` }} /></div><b>{money.format(item.value)}</b></div>)}{!result.costDistribution.length && <p className="operational-empty">Sem despesas no período.</p>}</div></article><article className="operational-panel"><div className="operational-panel-title"><div><span className="panel-kicker">Monitoramento</span><h2>Alertas priorizados</h2><p>Desvios ordenados por urgência e impacto.</p></div></div><div className="alert-list">{result.alerts.map((item) => <div className={item.level} key={item.id}><span aria-hidden="true">{item.level === "critical" ? "!" : item.level === "warning" ? "↗" : "i"}</span><p><small>{item.category}</small><strong>{item.title}</strong>{item.message}</p>{item.metric != null && <b>{formatAlertMetric(item)}</b>}</div>)}{!result.alerts.length && <p className="operational-empty">Nenhum alerta relevante para o filtro atual.</p>}</div></article></section>
