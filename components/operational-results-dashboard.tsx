...
import RecordActionGrid, { type RecordAction } from "./record-action-grid";
...
type Alert = { id: string; level: "info" | "warning" | "critical"; category: string; title: string; message: string; metric?: number; metricFormat?: "money" | "percent" | "number" };
type Insight = { id: string; priority: "high" | "medium" | "opportunity"; category: string; title: string; description: string; evidence: string; action?: Exclude<Dialog, "contractor" | "expense" | "settings" | null>; actionLabel?: string };
type Result = {
  filters: Filters;
  settings: Settings;
  totals: Totals;
  ...
  monthly: Breakdown[];
  billingPipeline: { predicted: number; billed: number; received: number; toBill: number; toCollect: number; overdue: number; billingRate: number | null; collectionRate: number | null };
...
function presetDates(preset: string) {
  ...
  if (preset === "LAST_6_MONTHS") {
    const start = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 5, 1, 12));
    return { startDate: iso(start), endDate: today };
  }
...
function ResultsSkeleton() {
  return <div className="operational-skeleton" role="status" aria-label="Calculando indicadores financeiros">
    ...
    <div className="executive-skeleton"><span className="skeleton-block chart" aria-hidden="true" /><span className="skeleton-block chart" aria-hidden="true" /></div>
  </div>;
}
...
function RecordDialog({ type, result, contractors, onClose, onSaved }: { type: Exclude<Dialog, null>; result: Result; contractors: Contractor[]; onClose(): void; onSaved(): Promise<void> }) {
  ...
  const descriptions = { contractor: "Cadastre uma empresa, órgão público ou cliente.", contract: "Defina as regras financeiras e operacionais do contrato.", maintenance: "Registre custos e serviços realizados no veículo.", expense: "Registre um novo custo da operação.", revenue: "Registre uma nova entrada financeira.", invoice: "Registre valores faturados ao contratante.", payment: "Informe um valor efetivamente recebido.", settings: "Ajuste cálculos, alertas e indicadores do painel." };
  const saveLabels = { contractor: "Salvar contratante", contract: "Salvar contrato", maintenance: "Salvar manutenção", expense: "Salvar despesa", revenue: "Salvar receita", invoice: "Salvar faturamento", payment: "Salvar recebimento", settings: "Salvar configurações" };
  function requestClose() {
    if (saving) return;
    if (!dirty.current || window.confirm("Descartar as alterações deste formulário?")) onClose();
  }
  ...
  return <MotionBackdrop open className="modal-backdrop" dismissOnBackdrop onDismiss={requestClose}>
    <form className="admin-modal operational-dialog" onSubmit={submit} onChangeCapture={() => { dirty.current = true; }} role="dialog" aria-modal="true" aria-labelledby="record-dialog-title">
      <div className="operational-dialog-heading"><div><span className="operational-eyebrow">Cadastros e lançamentos</span><h2 id="record-dialog-title">{titles[type]}</h2><p>{descriptions[type]} O registro será associado somente à empresa autenticada.</p></div><button className="modal-close" type="button" onClick={requestClose} aria-label="Fechar formulário">×</button></div>
      ...
      <div className="modal-actions operational-dialog-footer"><span>* campos obrigatórios</span><button className="secondary-action" type="button" disabled={saving} onClick={requestClose}>Cancelar</button><button className="primary-action" disabled={saving} aria-busy={saving}>{saving ? <span className="button-progress"><span className="button-spinner" />Salvando…</span> : saveLabels[type]}</button></div>
    </form>
  </MotionBackdrop>;
}
...
export default function OperationalResultsDashboard({ variant = "result" }: { variant?: "dashboard" | "result" | "records" }) {
  ...
  const formatAlertMetric = (item: Alert) => item.metricFormat === "money" ? money.format(Number(item.metric))
    : item.metricFormat === "percent" ? `${numberFormatter.format(Number(item.metric))}%`
      : `${numberFormatter.format(Number(item.metric))} km`;
  const openRecord = (type: RecordAction) => setDialog(type);
  const successMessages: Record<Exclude<Dialog, null>, string> = {
    revenue: "Receita cadastrada com sucesso.", expense: "Despesa cadastrada com sucesso.", invoice: "Faturamento cadastrado com sucesso.", payment: "Recebimento cadastrado com sucesso.", maintenance: "Manutenção cadastrada com sucesso.", contract: "Contrato cadastrado com sucesso.", contractor: "Contratante cadastrado com sucesso.", settings: "Configurações salvas com sucesso.",
  };
  async function recordSaved(type: Exclude<Dialog, null>) {
    await load(filters);
    setToast(`${successMessages[type]} Indicadores e gráficos atualizados.`);
  }

  if (variant === "records") return <main className="operational-page records-page">
    <header className="operational-title records-title"><div><span className="operational-eyebrow">Dados reais · {organization || "sua empresa"}</span><h1>Cadastros e Lançamentos</h1><p>Registre receitas, despesas, contratos e demais movimentações da operação.</p></div><div className="operational-actions"><ContextualHelp articleId="resultado-operacional" /><Link className="secondary-action" href="/">Voltar ao Dashboard</Link></div></header>
    {error && <div className="form-error operational-notice" role="alert">{error}</div>}
    {loading ? <div className="records-page-skeleton" role="status" aria-label="Carregando cadastros e lançamentos"><span className="skeleton-block"/><span className="skeleton-block"/><span className="skeleton-block"/><span className="skeleton-block"/></div> : result && <><RecordActionGrid onAction={openRecord} counts={{ contracts: result.details.contracts.length, invoices: result.details.invoices.length, payments: result.details.payments.length, refuelings: result.details.refuelings.length }} /><section className="operational-panel records-latest"><div className="operational-panel-title"><div><span className="panel-kicker">Atualização imediata</span><h2>Últimas movimentações</h2><p>Depois de salvar, os novos valores aparecem aqui e na Dashboard sem F5.</p></div></div><div className="movement-list">{result.latestMovements.map((item) => <div key={`${item.origin}-${item.id}`}><span className={item.value >= 0 ? "entry" : "exit"}>{item.value >= 0 ? "+" : "−"}</span><p><strong>{item.description}</strong><small>{item.date} · {label(item.origin)}{item.vehicle ? ` · ${item.vehicle}` : ""}</small></p><b className={item.value >= 0 ? "good" : "bad"}>{money.format(Math.abs(item.value))}</b></div>)}{!result.latestMovements.length && <p className="operational-empty">Ainda não há movimentações. Use uma das ações acima para começar.</p>}</div></section></>}
    {toast && <div className="operational-toast" role="status"><span aria-hidden="true">✓</span><p>{toast}</p><button type="button" onClick={() => setToast("")} aria-label="Fechar notificação">×</button></div>}
    {dialog && result && <RecordDialog type={dialog} result={result} contractors={contractors} onClose={() => setDialog(null)} onSaved={() => recordSaved(dialog)} />}
  </main>;

  return <main className="operational-page">
    <header className="operational-title"><div><span className="operational-eyebrow">Dados reais · {organization}</span><h1>{variant === "dashboard" ? "Dashboard financeiro" : "Resultado Operacional"}</h1><p>Contratos, faturamentos, recebimentos, KM e custos consolidados automaticamente.</p></div><div className="operational-actions"><ContextualHelp articleId="resultado-operacional" /><Link className="secondary-action" href="/contratos">Gerenciar contratos</Link><button className="secondary-action" onClick={() => setDialog("settings")} disabled={!result}>Configurações</button><button className="secondary-action" onClick={exportExcel} disabled={!result || saving === "excel"}>{saving === "excel" ? "Gerando…" : "Exportar Excel"}</button><button className="primary-action" onClick={closePeriod} disabled={!result || saving === "closing" || Boolean(result?.snapshotMeta)}>{saving === "closing" ? "Fechando…" : result?.snapshotMeta ? "Período fechado" : "Fechar período"}</button></div></header>
    <div className="preset-bar" aria-label="Períodos rápidos"><span>Período</span>{[["TODAY", "Hoje"], ["THIS_WEEK", "Esta semana"], ["THIS_MONTH", "Este mês"], ["PREVIOUS_MONTH", "Mês anterior"], ["LAST_6_MONTHS", "Últimos 6 meses"], ["THIS_YEAR", "Este ano"]].map(([value, title]) => <button type="button" key={value} disabled={loading} onClick={() => void applyPreset(value)}>{title}</button>)}</div>
    ...
    {loading ? <ResultsSkeleton /> : result && <>
      ...
      <section className="operational-card-grid" aria-label="Indicadores do período">{cards.map((card) => <button type="button" className={`operational-kpi${card.key === "result" || card.key === "accumulated" ? Number(card.value) >= 0 ? " positive" : " negative" : ""}`} key={card.key} onClick={() => setDetail(card.key)}><span title={card.key === "result" ? "Receitas consideradas menos despesas do período." : card.key === "contractedKm" ? "Franquia de quilômetros definida nos contratos filtrados." : card.key === "excessKm" ? "Quilômetros realizados acima da franquia contratada." : undefined}>{card.label}{["result", "contractedKm", "excessKm"].includes(card.key) && <i className="kpi-help-mark" aria-hidden="true">?</i>}</span><strong>{card.format === "money" ? money.format(Number(card.value)) : card.format === "percent" ? `${numberFormatter.format(Number(card.value))}%` : `${numberFormatter.format(Number(card.value))}${card.key.toLocaleLowerCase().includes("km") ? " km" : ""}`}</strong><small>Ver detalhes</small></button>)}</section>
      <FinancialCharts monthly={result.monthly.map((item) => ({ month: String(item.month), revenue: Number(item.revenue), expenses: Number(item.expenses), operationalResult: Number(item.operationalResult) }))} costs={result.costDistribution} money={money} onAction={openRecord} />
      <section className="executive-grid billing-only" aria-label="Faturamento"><article className="operational-panel billing-panel"><div className="operational-panel-title"><div><span className="panel-kicker">Funil de faturamento</span><h2>Da previsão ao caixa</h2><p>Veja onde a receita ainda precisa avançar.</p></div></div><div className="billing-funnel">{[["Receita prevista", result.billingPipeline.predicted, 100], ["Faturado", result.billingPipeline.billed, result.billingPipeline.billingRate ?? 0], ["Recebido de faturas", result.billingPipeline.received, result.billingPipeline.predicted ? result.billingPipeline.received / result.billingPipeline.predicted * 100 : 0]].map(([title, value, percent]) => <div key={String(title)}><span><strong>{String(title)}</strong><small>{money.format(Number(value))}</small></span><div><i style={{ transform: `scaleX(${Math.min(1, Number(percent) / 100)})` }} /></div></div>)}</div><div className="pipeline-gaps"><div className={result.billingPipeline.toBill > 0 ? "attention" : "ok"}><span>A faturar</span><strong>{money.format(result.billingPipeline.toBill)}</strong><small>{result.billingPipeline.billingRate == null ? "Sem receita prevista" : `${numberFormatter.format(result.billingPipeline.billingRate)}% faturado`}</small></div><div className={result.billingPipeline.toCollect > 0 ? "attention" : "ok"}><span>A receber do faturado</span><strong>{money.format(result.billingPipeline.toCollect)}</strong><small>{result.billingPipeline.collectionRate == null ? "Sem faturas emitidas" : `${numberFormatter.format(result.billingPipeline.collectionRate)}% recebido`}</small></div></div></article></section>
      <section className="operational-panel insight-panel"><div className="operational-panel-title"><div><span className="panel-kicker">Insights acionáveis</span><h2>O que fazer para melhorar</h2><p>Sugestões calculadas com os dados registrados. Valide documentos e regras do contrato antes de agir.</p></div><span className="insight-count">{result.insights.length} recomendações</span></div><div className="insight-grid">{result.insights.map((item) => <article className={`insight-card ${item.priority}`} key={item.id}><div><span>{item.category}</span><b>{item.priority === "high" ? "Prioridade" : item.priority === "medium" ? "Atenção" : "Oportunidade"}</b></div><h3>{item.title}</h3><p>{item.description}</p><small>{item.evidence}</small>{item.action && <button type="button" onClick={() => setDialog(item.action ?? null)}>{item.actionLabel}<span aria-hidden="true">→</span></button>}</article>)}</div></section>
      <section className="operational-panel"><div className="operational-panel-title"><div><span className="panel-kicker">Monitoramento</span><h2>Alertas priorizados</h2><p>Desvios ordenados por urgência e impacto.</p></div></div><div className="alert-list">{result.alerts.map((item) => <div className={item.level} key={item.id}><span aria-hidden="true">{item.level === "critical" ? "!" : item.level === "warning" ? "↗" : "i"}</span><p><small>{item.category}</small><strong>{item.title}</strong>{item.message}</p>{item.metric != null && <b>{formatAlertMetric(item)}</b>}</div>)}{!result.alerts.length && <p className="operational-empty">Nenhum alerta relevante para o filtro atual.</p>}</div></section>
      ...
      <section className="operational-two-columns"><article className="operational-panel"><div className="operational-panel-title"><div><h2>Últimas movimentações</h2><p>Origem explícita evita duplicidade.</p></div></div><div className="movement-list">{result.latestMovements.map((item) => <div key={`${item.origin}-${item.id}`}><span className={item.value >= 0 ? "entry" : "exit"}>{item.value >= 0 ? "+" : "−"}</span><p><strong>{item.description}</strong><small>{item.date} · {label(item.origin)}{item.vehicle ? ` · ${item.vehicle}` : ""}</small></p><b className={item.value >= 0 ? "good" : "bad"}>{money.format(Math.abs(item.value))}</b></div>)}{!result.latestMovements.length && <p className="operational-empty">Sem movimentações no período.</p>}</div></article><article className="operational-panel"><div className="operational-panel-title"><div><h2>Cadastros e lançamentos</h2><p>Novos fatos atualizam o painel sem recadastro.</p></div><Link className="table-action" href="/lancamentos">Abrir área completa</Link></div><RecordActionGrid compact onAction={openRecord} counts={{ contracts: result.details.contracts.length, invoices: result.details.invoices.length, payments: result.details.payments.length, refuelings: result.details.refuelings.length }} /></article></section>
