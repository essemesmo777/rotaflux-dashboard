...
export default function AdminPage() {
  ...
  useEffect(() => { load(); }, [load]);
  const stats = useMemo(() => ({ total: users.length, active: users.filter((user) => user.status === "ACTIVE").length, admins: users.filter((user) => user.role === "SUPER_ADMIN").length }), [users]);
  ...
  return (
    <main className="admin-page">
      <header className="admin-header"><div className="auth-brand"><span className="brand-mark">R</span><span>RotaFlux Admin</span></div><nav><a href="/">Dashboard</a><button onClick={logout}>Sair</button></nav></header>
