...
export default function OperationsPage() {
  ...
  const [frameSource, setFrameSource] = useState("/operations.html");

  useEffect(() => {
    let active = true;
    Promise.resolve().then(() => {
      if (active) setFrameSource(`/operations.html${window.location.search}`);
    });
    ...
    return (
      <main className="session-screen">
        <div className="session-card" role="status">
          ...
          <strong>{state === "loading" ? "Carregando operações…" : "Acesso indisponível"}</strong>
          {message && <p>{message}</p>}
  ...
  return <main className="site-shell"><iframe className="dashboard-frame" src={frameSource} title="Operações RotaFlux" /></main>;
}
