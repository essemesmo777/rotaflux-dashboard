...
export async function GET(request: Request) {
  ...
  if (!session) return Response.json({ error: "Sessão expirada." }, { status: 401, headers: noStore });
  if (!canManageCompany(session.profile.role)) return Response.json({ error: "Acesso financeiro restrito aos administradores." }, { status: 403, headers: noStore });
  ...
  if (filters.endDate < filters.startDate) return Response.json({ error: "O fim do período deve ser posterior ao início." }, { status: 400, headers: noStore });
  try {
    const result = await operationalResultFor(session.token, session.profile.organization_id, filters);
    return Response.json({ result, organization: session.profile.organizations?.name ?? "Minha empresa" }, { headers: noStore });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Não foi possível calcular o resultado operacional." }, { status: 500, headers: noStore });
  }
}
